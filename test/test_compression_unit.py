...
class CompressionUnitTests(VerboseTestCase):
    ...
    def test_compression_updates_existing_metrics_row_with_mocked_tools(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertTrue((logdir / "timings" / "compression" / "rdf" / "gzip.txt").exists())
            self.assertTrue((logdir / "timings" / "compression" / "rdf" / "brotli.txt").exists())
            self.assertTrue((logdir / "timings" / "compression" / "rdf" / "hdt.txt").exists())
            self.assertTrue((logdir / "stages" / "compression" / "rdf.json").exists())
    ...
    def test_compression_backs_up_metrics_file_on_header_mismatch(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(result.returncode, 0, msg=result.stderr)
            self.assertTrue((logdir / "metrics_csv_bak_run-hdr.csv").exists())
