...
Run inside the VCF-RDFizer image, where every engine exists:
...
first argument (or in VCF_RDFIZER_AGREEMENT_ENGINES) to narrow it, which is
useful when one engine is slow: Comunica answers each query in about a second
against N-Triples or HDT, while QLever and COTTAS answer in milliseconds once
their artifact exists.
...
V = load_runner()
DEFAULT_ENGINES = V.SPARQL_ENGINES


def selected_engines(argv: list[str]) -> tuple[str, ...]:
    """Engines to compare: the argument, then the environment, then all of them."""
    raw = ""
    if len(argv) > 1:
        raw = argv[1]
    else:
        raw = os.environ.get("VCF_RDFIZER_AGREEMENT_ENGINES", "")
    if not raw.strip():
        return tuple(DEFAULT_ENGINES)
    return tuple(V.parse_engine_list(raw))
...
def evaluate(engine_name: str, representation: str, source: Path, scratch: Path) -> dict:
    ...
    # A private scratch per run: the native engines name their built artifact
    # after the engine, so a shared directory would have HDT reading the graph
    # it built for the other representation.
    run_scratch = scratch / f"scratch-{engine_name}-{representation}"
    run_scratch.mkdir(parents=True, exist_ok=True)
    engine = V.build_engine(
        engine_name, source, raw_dir=raw_dir, scratch=run_scratch,
...
def verdict_for(engine_name: str, representation: str, source: Path, scratch: Path) -> dict:
    ...
    raw_dir.mkdir(parents=True, exist_ok=True)
    run_scratch = scratch / f"verdict-scratch-{engine_name}-{representation}"
    run_scratch.mkdir(parents=True, exist_ok=True)
    engine = V.build_engine(
        engine_name, source, raw_dir=raw_dir, scratch=run_scratch,
        options={"memory_gb": 2, "startup_timeout": 300, "query_timeout": 300},
...
def main(argv: list[str] | None = None) -> int:
    ...
    engines = selected_engines(argv if argv is not None else sys.argv)
    print("Comparing: " + ", ".join(engines))
    disagreements = 0
